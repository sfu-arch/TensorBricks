...
import node.{FPmatNxN, FPvecN, matNxN, vecN}
import shell._
...
/** Core.
  ...
  * directory.
  */
class DNNCorePDP(Hx: Int = 3, Wx: Int = 3, Cx: Int = 2, Cb: Int = 2, Fx: Int = 1, Px: Int = 1, K: Int = 3,
...
//
  ...
  val macDWShape = new matNxN(K, false)
  val macPW2Shape = new FPvecN(Fx, S, 0)

  val wgtDWShape = new FPvecN(K * K, S, 0)
  val wgtPW2Shape = new FPvecN(Fx, S, 0)
...
//  val conv = Module(new PW_Block(MACperCH, Fx, 5, "wgtPW", "inp")(memShape)(CxShape))
  val conv = Module(new PDP_Block(Hx, K, Fx, Cb, Px,
