...
verilog: $(chisel_build_dir)/$(TOP)_$(Hx).v
$(chisel_build_dir)/$(TOP)_$(Hx).v: $(shell find . -type f -name "*.scala" | sed 's/ /\\ /g')#install_vta_package
	sbt 'test:runMain accel.${TOP}Main --target-dir $(chisel_build_dir) --top-name $(TOP)_$(Hx) --CONV $(CONV) --Hx $(Hx) --Wx $(Wx) --Cx $(Cx) --Cb $(Cb) --Fx $(Fx) --Px $(Px) --K $(K) --num-ptrs $(NUM_PTRS) --num-vals $(NUM_VALS) --num-rets $(NUM_RETS) --num-events $(NUM_EVENTS) --num-ctrls $(NUM_CTRLS)'
	# sbt 'test:runMain accel.${TOP}Main --target-dir $(chisel_build_dir) --top-name $(TOP) --num-ptrs $(NUM_PTRS) --num-vals $(NUM_VALS)'
