...
class ShapeTransformerIO[gen <: Shapes, gen2 <: Shapes](NumIns: Int, NumOuts: Int)(shapeIn: => gen)(shapeOut: => gen2)(implicit p: Parameters)
...
class ShapeTransformer[L <: Shapes, K <: Shapes](NumIns: Int, NumOuts: Int, ID: Int)(shapeIn: => L)(shapeOut: => K)(implicit p: Parameters)
  ...
  val dataIn_Wire = Wire(Vec(shapeIn.getLength(), Vec (NumIns, UInt(xlen.W))))

  val input_data = dataIn_R.map(_.data.asUInt())


  for (i <- 0 until shapeIn.getLength()) {
  ...
  val mux = Module(new Mux(new CustomDataBundle(UInt((NumIns * xlen).W)), shapeIn.getLength()))
  val countOn = RegInit(init = false.B)
  val cnt = Counter(shapeIn.getLength())
  ...
  when (cnt.value === (shapeIn.getLength() - 1).U) {
