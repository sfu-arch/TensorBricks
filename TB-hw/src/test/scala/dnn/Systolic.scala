...
class SystolicTests(df: SystolicBLAS[UInt])(implicit p: Parameters) extends PeekPokeTester(df) {
  poke(df.io.activate, false.B)
  // left * right
  df.io.left.zipWithIndex.foreach { case (io, i) => poke(io, (i + 1).U) }
  df.io.right.zipWithIndex.foreach { case (io, i) => poke(io, (i + 1).U) }
...
class SystolicTestStream(df: SystolicSquareWrapper[UInt])(implicit p: Parameters) extends PeekPokeTester(df) {
