...
class HWCF_SchedulePDP(Schedule):
    ...
    def __init__(self, hw_type, second_pw_dataflow, net, model_name, result_dir, verbose,  hardware_yaml=None, hardware_dict=None):
        ...
        self.conv2d_pw = hwcf_conv2d_pw
    def __str__(self):
        return 'hwcf_hwc_{}_schedule_pdp_{}'.format(self.second_pw_dataflow, self.hw_type)
...
#-------------------------------------------
            ...
            if current_layer.attr_type == 'PW':
                ...
                self.conv2d_pw(self, current_layer, pw_layer_hw_params)
            ...
            if current_layer.attr_type == '3d':
                ...
                self.conv2d_pw(self, current_layer, per_layer_hw_params)
                self.layer_names.append(current_layer.name)
            ...
            for win in range(0, first_layer.Win, first_layer_hw_params.wxx):
                ...
                for cin in range(0, first_layer.Cin, first_layer_hw_params.cxx):
                    ...
                    pw_block_start_indices_1 = AttrDict({'orig_hin': hin, 'end_hin': end_hin_idx+1,
                                                       ...
                                                       'cout':0, 'cout_end': num_cout})

                    cycles_1  = hwcf_pw_block(self, pw_block_start_indices_1, first_layer,
                ...
                # end cin
                # At this point input = Hx*Wx*C and output is [Hx*Wx*F] --> Need to calculate stuff Hx Wx Fx at a time
                # --------------------------------------------------------------------------------------------------
                ...
                dw_start_indices_2 = AttrDict({'hin': hin, 'win': win, 'cin': 0,
                                               'hout': start_hout_idx, 'wout': start_wout_idx})
                cycles_2 = hwc_conv2d_dw(self, second_partial_layer, second_layer_hw_params,
                                         dw_start_indices_2, num_cross_layers=3, layer_position_idx=1)
