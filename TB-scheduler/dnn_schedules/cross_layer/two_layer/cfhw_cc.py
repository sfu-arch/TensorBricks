...
class CFHWScheduleCC(Schedule):
    ...
    def __str__(self):
        return 'cfhw_{}_schedule_cc_{}'.format(self.second_pw_dataflow, self.hw_type)
...
def conv_conv(cls, first_layer, second_layer):
    ...
    for cin in range(0, first_layer.Cin, first_layer_hw_params.cxx):
        ...
        end_cout_idx, num_cout = cls.f_params_calculation(0, first_layer_hw_params, first_layer.Cout)
        cls.debug_message('=== Layer 1: PW ===')
        pw_block_start_indices_1 = AttrDict({'hin': 0, 'end_hin': first_layer.Hin,
                                             'win': 0, 'end_win': first_layer.Win,
                                             'cin': cin, 'end_cin': first_layer.Cin,
                                             'hout': 0, 'wout': 0,
                                             'cout': 0, 'end_cout': end_cout_idx + 1})
