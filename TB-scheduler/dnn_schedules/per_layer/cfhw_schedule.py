...
def conv2d_pw(cls, layer_attr, hw_params, pw_start_indices=None, num_cross_layers=1,
    ...
    # TODO: Add padding, LAYER_WOUT = int((LAYER_WIN - layer_attr.Ky + 2*Py) / layer_attr.Sy) + 1
...
######################################################
    ...
    for orig_cin in range(INIT_START_CIN_IDX, INIT_END_CIN, hw_params.cxx):
        ...
        current_cycles_all_filters = 0
        for f in range(INIT_START_COUT_IDX, INIT_END_COUT, hw_params.fx):


            # ------ f parameter calculations
            orig_hout = INIT_START_HOUT_IDX
            end_f_idx, num_f = cls.f_params_calculation(f, hw_params, INIT_END_COUT)
            for orig_hin in range(INIT_START_HIN_IDX, INIT_END_HIN, hw_params.hxx):
                ...
                for orig_win in range(INIT_START_WIN_IDX, INIT_END_WIN, hw_params.wxx):
                    ...
                    # --------------------------------------
                    util_cycles_all_filters += mac_util_cycles_block
                    mac_cycles_all_filters += mac_cycles_block
                    dma_cycles_all_filters += dma_cycles_block
                    # -- padd logic --
                    ...
                    if orig_cin != 0 and layer_attr.attr_type != 'DW':
                        padd_util_cycles_all_filters += padd_util_cycles_block
                        padd_cycles_all_filters += padd_cycles_block
                    else:
                        padd_util_cycles_block = 0
                        padd_cycles_block = 0
        ...
        if orig_cin != 0 and layer_attr.attr_type != 'DW':
            ...
            cls.stats['padd_ops'][layer_attr.layer_idx] += LAYER_COUT * LAYER_WOUT * LAYER_HOUT
            cls.insert_max_stats('mem_partial_product', layer_attr.layer_idx, LAYER_COUT * LAYER_WOUT * LAYER_HOUT)

        # cycles information
        current_cycles_all_filters += cls.add_pw_block_stats(layer_attr, dma_cycles_all_filters,
                                                        util_cycles_all_filters,
                                                        mac_cycles_all_filters, padd_util_cycles_all_filters,
                                                        padd_cycles_all_filters)
        current_cycles += current_cycles_all_filters
    # end cin


    if num_cross_layers == layer_position_idx + 1 and INIT_END_CIN == layer_attr.Cin and INIT_END_COUT == layer_attr.Cout:
        isdma = True
        mem_out_act = LAYER_COUT * LAYER_WOUT * LAYER_HOUT
        cls.mem_out_act_stats(layer_attr, isdma, mem_out_act, 0, LAYER_HOUT-1, 0,
                              LAYER_WOUT-1, INIT_START_COUT_IDX, INIT_END_COUT - 1)
