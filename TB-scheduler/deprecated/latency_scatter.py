...
model_names = ['mobilenet_v2', 'mnasnet1_0','mobilenet'] #'xception','nasnetamobile']
...
# df['schedule_name']
# df['schedule_name'].replace('per_layer','Base')
# df['schedule_name'].replace(['per_layer','dp','pdp'],['Base','DP-PT','PT-DP-PT'])
df['schedule_name'] = df['schedule_name'].replace(['per_layer','dp','pdp'],['Base','DP-PT','PT-DP-PT'])
fig, ax = plt.subplots(figsize=(12, 8))
plt.gcf().subplots_adjust(bottom=0.15, right=0.99, top=0.88)
...
marker = {'PT-DP-PT':'o','DP-PT':'x','Base':'+'}
for sch in list(set(df_sorted['schedule_name'])):
    ...
    # ax.scatter(vals['total_dma_accesses'],vals['total_mac_cycles'],
    #             s=vals['total_memory_mb']*1024*3, marker = marker[sch],edgecolor='k', color=cl[sch],label=sch,
...
plt.legend(bbox_to_anchor=(1.04, 1.20), ncol=3, fontsize=35)
