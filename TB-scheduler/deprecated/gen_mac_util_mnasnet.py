...
def get_feature_pdp(i,j,k,df):
    if not (df['attr_type'].iloc[i] == 'PW' and df['attr_type'].iloc[j] == 'DW' and df['attr_type'].iloc[k] == 'PW'):
        raise Exception('Wrong indices')
    cumm_mac_cycles = df['mac_cycles'].iloc[i] + df['mac_cycles'].iloc[j] + df['mac_cycles'].iloc[k]
    pw1_cycle = df['cycles_total'].iloc[i]
    dw_cycle = df['cycles_total'].iloc[j]
    pw2_cycle = df['cycles_total'].iloc[k]
...
def get_average_cycle_mac_util(df,model_name):
    if model_name != 'mnasnet1_0':
        raise Exception('Wrong network: only mnasnet supported')
    ...
    early_layer_list = [(2,3,4),(5,6,7)]
    middle_layer_list = [(23,24,25)]
    late_layer_list = [(47,48,49)]

    for i,j,k in early_layer_list:
        print('layer : {}'.format(i))

        pw1_cycle, dw_cycle, pw2_cycle, cumm_mac_cycles, cumm_cycles = get_feature_pdp(i,j,k,df)
        cycles[0] += pw1_cycle
        cycles[1] += dw_cycle
        cycles[2] += pw2_cycle
    ...
    for i, j,k in middle_layer_list:
        # print('layer : {}'.format(i))
